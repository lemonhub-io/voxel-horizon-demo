import { U } from './utils';
import type { AudioToneOpts, AudioNoiseOpts, LoopHandle, LoopEntry } from './types';
...
export class AudioEngine {
  ...
  padTimer: ReturnType<typeof setInterval> | null = null;
  pluckTimer: ReturnType<typeof setInterval> | null = null;
  chordIdx: number = 0;
  ...
  makeNoise(dur: number): AudioBuffer {
  ...
  uiClick(): void { this.tone({ type: 'sine', f: 1700, f2: 2300, dur: 0.06, vol: 0.12, r: 0.05, rev: 0.2 }); this.tone({ type: 'square', f: 850, dur: 0.03, vol: 0.03 }); }
  uiOpen(): void {
  ...
  uiClose(): void { this.noise({ type: 'highpass', f: 1400, dur: 0.1, vol: 0.06 }); this.tone({ type: 'sine', f: 700, f2: 420, dur: 0.12, vol: 0.09 }); }
  uiDeny(): void { this.tone({ type: 'square', f: 220, dur: 0.09, vol: 0.09 }); this.tone({ type: 'square', f: 185, dur: 0.12, vol: 0.09, at: 0.1 }); }
  ...
  scanFound(i: number): void { this.tone({ type: 'sine', f: 1560 + i * 120, dur: 0.06, vol: 0.07, at: 0.3 + i * 0.07, rev: 0.5 }); }
  analyze(): void {
  ...
  analyzeTick(p: number): void { this.tone({ type: 'sine', f: 900 + p * 800, dur: 0.03, vol: 0.04 }); }
  recharge(): void {
  ...
  useItem(): void { this.tone({ type: 'sine', f: 620, f2: 1240, dur: 0.25, vol: 0.11, rev: 0.3 }); }
  overheat(): void {
    this.noise({ type: 'highpass', f: 2500, dur: 0.7, vol: 0.14, r: 0.4 });
    this.tone({ type: 'square', f: 340, f2: 180, dur: 0.3, vol: 0.08 });
  }
  splash(): void { this.noise({ type: 'lowpass', f: 1100, dur: 0.25, vol: 0.22, r: 0.15 }); }
  takeoff(): void {
  ...
  creatureHurt(pan: number): void { this.tone({ type: 'square', f: 500, f2: 260, dur: 0.14, vol: 0.1, pan, filter: 'lowpass', ff: 1800 }); }
  thunder(): void {
  ...
  stopMusic(): void {
    ...
    if (this.pluckTimer !== null) clearInterval(this.pluckTimer);
